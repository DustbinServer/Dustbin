...
app.listen(3000, (err: Error | null, address: string) => {
    if (err) {
        app.log.error(err);
        process.exit(1);
    }
    console.log(`Dustbin Init On ${address}`);
});
...
const start = async () => {
    try {
      ...
      console.log(`[ Info ] Server Started At http://localhost:${process.env.PORT || 1134}`);
    } catch (err) {
      console.error(err);
      process.exit(1);
    }
